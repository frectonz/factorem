...
The existence of null has been the oldest and the most notorious source of runtime errors. Null
has plagued our code with a never ending chain of null checks. It is time we eliminate null by
using techniques that have been shown in languages like **Rust** and **Haskell**.
...
types of null, **_null_** and **_undefined_**.
...
For example let's look at a **_Map_**.
...
get method returned an undefined and due to undefined being a **_falsey_** value in javascript
...
is a **_falsey_** value, so even though the get method returned a value the value returned was also
**_falsey_** thus the if condition didn't get executed.
...
The option type represents an optional value. An option is either a **_Some_** that contains a value or a
**_None_** that does not. The option type is used in places where in normal circumstances you get returned
